...
import com.eigenroute.portfoliosimulation.{FirstEstimateQuantitiesToAcquireCalculator, PortfolioFixture}
...
class FirstEstimateQuantitiesToAcquireCalculatorUTest
  ...
  with EstimatedQuantitiesToAcquire {
    ...
    .firstEstimateQuantitiesToAcquire(portfolioDesign, portfolioSnapshotZeroQuantity, 0.0011, 0d, 10d, 0d, 10040d)
    .map { est =>
  ...
  private def checkFirstEstimateQuantitiesToAcquire(
    ...
    .map { est =>
      PortfolioQuantityToAcquire(
        est.eTFCode, est.quantityToAcquire, round(est.effectivePrice), round(est.fractionalQuantity))
    } should contain theSameElementsAs expectedFirstEstimateQuantities
